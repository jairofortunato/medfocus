/**
 * Import FAMERP 2020 PDF into data/exams/famerp-2020.json + Supabase.
 *
 * This PDF has:
 * - 80 multiple-choice questions (A-D, no E)
 * - "Questão N" labels with specialty tags
 * - Answer key on the last page
 * - Watermark text "t.me/medicinalivre" throughout
 * - OCR artifacts (M→fi, etc.)
 */
...
const EXAM_NAME = 'FAMERP 2020';
const EXAM_SLUG = 'famerp-2020';
...
function fixTypography(text: string): string {
  return text
    ...
    .replace(/4\s*000\d{4}\s*\d{1,2}\s*/g, '') // comment IDs like "4 0001278 17"
    // Fix common PDF OCR artifacts
    .replace(/M(?=[a-záàãâéêíóôõúç])/g, 'fi') // M before lowercase = fi ligature
    .replace(/identif(?:i)?cou/gi, 'identificou')
    .replace(/espec[íi]f(?:i)?co/gi, 'específico')
    .replace(/prof(?:i)?ssional/gi, 'profissional')
    .replace(/bene(?:f)(?:í)?cio/gi, 'benefício')
    .replace(/signif(?:i)?cativo/gi, 'significativo')
    .replace(/dif(?:i)?culdade/gi, 'dificuldade')
    // Fix known FAMERP artifacts
    .replace(/Infeção/g, 'Infecção')
    .replace(/Sindrome/g, 'Síndrome')
    .replace(/cárdio respiratória/g, 'cardiorrespiratória')
    .replace(/imunoglubulina/g, 'imunoglobulina')
...
async function main() {
  ...
  const pdfPath = 'C:/Users/usuario/Downloads/FAMERP 2020.pdf';
  ...
  const ano = yearMatch ? parseInt(yearMatch[0]) : 2020;
